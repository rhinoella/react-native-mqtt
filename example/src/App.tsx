...
export default function App() {
  ...
  const [options, setOptions] = React.useState<MqttOptions>({
  ...
  React.useEffect(() => {
    ...
    client
      .init()
      .then(() => {
        client.on(MqttEvent.CONNECTION_LOST, (msg, code, trace) => {
          setLog(`Error code ${code}: ${msg}\n${trace}`);
        });

        client.on(MqttEvent.MESSAGE_RECEIVED, (topic, message) => {
